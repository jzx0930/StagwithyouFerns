...
- **個體夾** = `#01`、`#02`…(**不改**)。個體夾也可帶暱稱:`#編號-暱稱`(中或英),例 `#04-彩虹野銀`、`#01-Auburn River`、`#14-鹿角有毒的野銀7號 SBD#7`;**也可能沒有連字號直接接**,例 `#15酋長`。**資料夾一律不改**;寫入 data.json 時拆成 `label="#編號"` + 新欄位 `name="暱稱"`,拆解要同時支援有/無連字號(regex `^(#\d+)[-\s]*(.*)$`;`(.*)` 空就是無暱稱)。
