...
1. **掃描 + 偵測變更**:掃 `C:\Users\MyUser\Google 雲端硬碟檔案串流\我的雲端硬碟\植物照片`(**不是 G:\,那是失效掛載**;bash 無法掛載,用 Glob/Read 或 Drive 連接器),和上次狀態(現有 data.json + 資料夾名)比對,分出:(A)新增/改名夾(名稱非英文-中文),(B)**舊植物補了新照片**——名稱沒變,要比對該株 data.json 照片數 vs Drive 該夾實際照片數才抓得到(注意檔名可能是 `2025:M:D` 冒號格式,別只掃 `2026*`;本機清單會快取時改用 Drive 連接器 search_files),(C)沒變則跳過。先列出「這次要處理的清單」再往下做。
2. **查名+附加腳本**:找出未符合「英文-中文」的資料夾(分類夾=`屬名-中文`、植物夾=`種名/園藝名-中文`;純英文如 Akki/Nano/E 不動),上網查名(原生種→種小名、品種→英文園藝名、交種→交種式;查不到維持中文並回報),把 `Rn '舊' '新'` **append** 到腳本(只加不刪)。都符合就跳到第 5 步。
