...
1. **掃描 + 偵測變更**:`Glob` 掃 `G:\我的雲端硬碟\植物照片`(bash **無法掛載 G:\**,只能用 Glob/Read),和上次狀態(現有 data.json + 資料夾名)比對,分出:(A)新增/改名夾(名稱非英文-中文),(B)**舊植物補了新照片**——名稱沒變,要比對該株 data.json 照片數 vs Drive 該夾實際照片數才抓得到,(C)沒變則跳過。先列出「這次要處理的清單」再往下做。
